// Генерирует страницы юнитов: units/<slug>.html (EN — основной язык, корень) и ru/units/<slug>.html (RU); en/units/* — редиректы
...
for (const lang of ['ru', 'en']) {
  const outDir = lang === 'en' ? path.join(root, 'units') : path.join(root, 'ru', 'units');
  ...
  for (const u of USC_UNITS) {
    ...
    if (lang === 'en') {
      ...
      html = html.replace(/<button type="button" data-lang="ru" class="is-active">RU<\/button><span>\/<\/span><button type="button" data-lang="en">EN<\/button>/g,
    ...
    if (lang === 'en') fs.writeFileSync(path.join(root, 'en', 'units', `${u.slug}.html`), redirectStub(`${BASE}units/${u.slug}.html`));
